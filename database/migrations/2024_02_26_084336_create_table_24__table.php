<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{       
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('table_24_', function (Blueprint $table) {
            $table->id();
            $table->string('talaba_fish')->nullable();
            $table->string('davlat_grant_summasi')->nullable();
            $table->string('nashr_etilgan_davlat')->nullable();
            $table->string('ilmiy_turnal_nomi')->nullable();
            $table->string('ilmiy_maqola_nomi')->nullable();
            $table->string('nashr_yili_betlari')->nullable();
            $table->string('index_url')->nullable();
            $table->string('mualliflar_soni')->nullable();
            $table->string('asos_file')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('table_24_');
    }
};
