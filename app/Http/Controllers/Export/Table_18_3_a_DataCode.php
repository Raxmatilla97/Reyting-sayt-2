...
class Table_18_3_a_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_18_3_a_record = $pointEntry->table_18_3_a; // Tablitsiya relation to'g'ri formatda

            if ($table_18_3_a_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_18_3_a_record->diplom_seriya_raqam ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_18_3_a_record->muassasa_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_18_3_a_record->shifr_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_18_3_a_record->buyruq_raqami_sanasi ?? 'N/A');

                    if ($table_18_3_a_record->asos_file) {
                        $sheet->setCellValue('H' . $row, 'Yuklash');
                        $sheet->getCell('H' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_18_3_a_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_18_3: $writtenRows");
    }
}