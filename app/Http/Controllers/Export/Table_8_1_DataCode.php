...
class Table_8_1_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_8_1_record = $pointEntry->table_8_1; // Tablitsiya relation to'g'ri formatda
            ...
            if ($table_8_1_record) {
                try {
                    ...
                    $sheet->setCellValue('D' . $row, $table_8_1_record->xorijiy_til_bilish_daraja ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_8_1_record->xorijiy_til_turi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_8_1_record->sertifikat_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_8_1_record->sertifikat_sanasi_raqami ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_8_1_record->sertifikat_muddati ?? 'N/A');

                    if ($table_8_1_record->asos_file) {
                        $sheet->setCellValue('I' . $row, 'Yuklash');
                        $sheet->getCell('I' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_8_1_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_8_1: $writtenRows");
    }
}