...
class Table_18_2_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_18_2_record = $pointEntry->table_18_2; // Tablitsiya relation to'g'ri formatda

            if ($table_18_2_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_18_2_record->diplom_seriya_raqam ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_18_2_record->muassasa_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_18_2_record->shifr_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_18_2_record->buyruq_raqami_sanasi ?? 'N/A');

                    if ($table_18_2_record->asos_file) {
                        $sheet->setCellValue('H' . $row, 'Yuklash');
                        $sheet->getCell('H' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_18_2_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_18_2: $writtenRows");
    }
}