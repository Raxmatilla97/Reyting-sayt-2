...
class Table_20_3_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_20_3_record = $pointEntry->table_20_3; // Tablitsiya relation to'g'ri formatda

            if ($table_20_3_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_20_3_record->mualliflar_soni ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_20_3_record->jurnal_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_20_3_record->asos_url ?? 'N/A');

                    if ($table_20_3_record->asos_file) {
                        $sheet->setCellValue('G' . $row, 'Yuklash');
                        $sheet->getCell('G' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_20_3_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_20_3: $writtenRows");
    }
}