...
class Table_2_3_1_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_2_3_1_record = $pointEntry->table_2_3_1;

            if ($table_2_3_1_record) {
                try {
                    // Nomer berish
                    ...
                    $sheet->setCellValue('C' . $row, $table_2_3_1_record->xorijiy_oqituvchi_ismi ?? 'N/A');
                    $sheet->setCellValue('D' . $row, $table_2_3_1_record->davlati_ish_joyi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_2_3_1_record->mutaxasisligi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_2_3_1_record->dars_beradigan_fani ?? 'N/A');

                    if ($table_2_3_1_record->asos_file) {
                        $sheet->setCellValue('G' . $row, 'Yuklash');
                        $sheet->getCell('G' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_2_3_1_record->asos_file));
                    } else {
