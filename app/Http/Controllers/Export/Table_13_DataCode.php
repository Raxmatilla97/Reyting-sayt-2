...
class Table_13_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_13_record = $pointEntry->table_13; // Tablitsiya relation to'g'ri formatda
            ...
            if ($table_13_record) {
                try {
                    ...
                    $sheet->setCellValue('D' . $row, $table_13_record->jurnal_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_13_record->maqola_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_13_record->yili_betlari ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_13_record->url_link ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_13_record->mualliflar_soni ?? 'N/A');
                    $sheet->setCellValue('I' . $row, $table_13_record->assos_url ?? 'N/A');

                    if ($table_13_record->asos_file) {
                        $sheet->setCellValue('J' . $row, 'Yuklash');
                        $sheet->getCell('J' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_13_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_13: $writtenRows");
    }
}