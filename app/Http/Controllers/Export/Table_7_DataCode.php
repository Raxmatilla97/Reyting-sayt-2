...
class Table_7_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_7_record = $pointEntry->table_7; // Tablitsiya relation to'g'ri formatda
            ...
            if ($table_7_record) {
                try {
                    ...
                    $sheet->setCellValue('D' . $row, $table_7_record->kurs_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_7_record->xorijiy_davlat_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_7_record->sertifikat_raqam_sana ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_7_record->link_url ?? 'N/A');

                    if ($table_7_record->asos_file) {
                        $sheet->setCellValue('H' . $row, 'Yuklash');
                        $sheet->getCell('H' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_7_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_7: $writtenRows");
    }
}