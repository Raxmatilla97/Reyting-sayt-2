...
class Table_4_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_4_record = $pointEntry->table_4; // Tablitsiya relation to'g'ri formatda
            ...
            if ($table_4_record) {
                try {
                    ...
                    $sheet->setCellValue('D' . $row, $table_4_record->ixtisoslik_shifr_nomi_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_4_record->mualiflar_soni ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_4_record->darslik_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_4_record->guvohnoma_raqami ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_4_record->darslik_restr_raqami ?? 'N/A');

                    if ($table_4_record->asos_file) {
                        $sheet->setCellValue('I' . $row, 'Yuklash');
                        $sheet->getCell('I' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_4_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_4: $writtenRows");
    }
}