...
class Table_11_1_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_11_1_record = $pointEntry->table_11_1; // Tablitsiya relation to'g'ri formatda

            if ($table_11_1_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_11_1_record->jurnal_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_11_1_record->nashr_yili ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_11_1_record->maqola_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_11_1_record->tili ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_11_1_record->index_url ?? 'N/A');
                    $sheet->setCellValue('I' . $row, $table_11_1_record->iqtiboslar_soni ?? 'N/A');
        ...
        // Log::info("Total rows written to table_11_1: $writtenRows");
    }
}