...
class Table_10_3_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_10_3_record = $pointEntry->table_10_3; // Tablitsiya relation to'g'ri formatda

            if ($table_10_3_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_10_3_record->davlat_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_10_3_record->konfrrensiya_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_10_3_record->maqola_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_10_3_record->Nashr_yili_betlari ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_10_3_record->url_link ?? 'N/A');
                    $sheet->setCellValue('I' . $row, $table_10_3_record->index_url ?? 'N/A');
                    $sheet->setCellValue('J' . $row, $table_10_3_record->mualliflar_soni ?? 'N/A');

                    if ($table_10_3_record->asos_file) {
                        $sheet->setCellValue('K' . $row, 'Yuklash');
                        $sheet->getCell('K' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_10_3_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_10_3: $writtenRows");
    }
}