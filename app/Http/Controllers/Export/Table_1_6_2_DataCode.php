...
class Table_1_6_2_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_1_6_2_record = $pointEntry->table_1_6_2;

            if ($table_1_6_2_record) {
                try {
                    // Nomer berish
                    ...
                    $sheet->setCellValue('E' . $row, $table_1_6_2_record->ilmiy_jurnal_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_1_6_2_record->ilmiy_maqola_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_1_6_2_record->nashr_yili_betlari ?? 'N/A');

                    // URL manzilini giperhavola qilish
                    $url = $table_1_6_2_record->url_manzili ?? 'N/A';
                    if ($url !== 'N/A') {
                    ...
                    $sheet->setCellValue('I' . $row, $table_1_6_2_record->mualliflar_soni ?? 'N/A');

                    if ($table_1_6_2_record->asos_file) {
                        $sheet->setCellValue('J' . $row, 'Yuklash');
                        $sheet->getCell('J' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_1_6_2_record->asos_file));
                    } else {
