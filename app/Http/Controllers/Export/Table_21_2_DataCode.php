...
class Table_21_2_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_21_2_record = $pointEntry->table_21_2; // Tablitsiya relation to'g'ri formatda

            if ($table_21_2_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_21_2_record->fish ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_21_2_record->musobaqa_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_21_2_record->otkazilgan_joy_sana ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_21_2_record->musobaqalar_nomi ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_21_2_record->egallagan_orin ?? 'N/A');
                    $sheet->setCellValue('I' . $row, $table_21_2_record->diplom_seriya_raqami ?? 'N/A');

                    if ($table_21_2_record->asos_file) {
                        $sheet->setCellValue('J' . $row, 'Yuklash');
                        $sheet->getCell('J' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_21_2_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_21_2: $writtenRows");
    }
}