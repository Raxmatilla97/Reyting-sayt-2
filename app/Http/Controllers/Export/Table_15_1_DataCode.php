...
class Table_15_1_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_15_1_record = $pointEntry->table_15_1; // Tablitsiya relation to'g'ri formatda

            if ($table_15_1_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_15_1_record->fond_buyurtma_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_15_1_record->fond_buyurtma_summa ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_15_1_record->jami_summa ?? 'N/A');

                    if ($table_15_1_record->asos_file) {
                        $sheet->setCellValue('G' . $row, 'Yuklash');
                        $sheet->getCell('G' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_15_1_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_15_1: $writtenRows");
    }
}