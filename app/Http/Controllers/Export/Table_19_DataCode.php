...
class Table_19_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_19_record = $pointEntry->table_19; // Tablitsiya relation to'g'ri formatda
            ...
            if ($table_19_record) {
                try {
                    ...
                    $sheet->setCellValue('D' . $row, $table_19_record->otm_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_19_record->ik_raqami ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_19_record->shifr_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_19_record->disertatsiya_mavzusi ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_19_record->oak_sana ?? 'N/A');

                    if ($table_19_record->asos_file) {
                        $sheet->setCellValue('I' . $row, 'Yuklash');
                        $sheet->getCell('I' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_19_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_19: $writtenRows");
    }
}