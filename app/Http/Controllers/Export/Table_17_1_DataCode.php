...
class Table_17_1_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_17_1_record = $pointEntry->table_17_1; // Tablitsiya relation to'g'ri formatda

            if ($table_17_1_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_17_1_record->patent_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_17_1_record->berilgan_sana ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_17_1_record->raqami ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_17_1_record->patent_url ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_17_1_record->huquq_egalari_soni ?? 'N/A');

                    if ($table_17_1_record->asos_file) {
                        $sheet->setCellValue('I' . $row, 'Yuklash');
                        $sheet->getCell('I' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_17_1_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_17_1: $writtenRows");
    }
}