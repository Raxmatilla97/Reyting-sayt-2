...
class Table_10_2_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_10_2_record = $pointEntry->table_10_2; // Tablitsiya relation to'g'ri formatda

            if ($table_10_2_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_10_2_record->nashr_davlat_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_10_2_record->ilmiy_jurnal_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_10_2_record->ilmiy_maqola_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_10_2_record->nashr_yili_betlari ?? 'N/A');
                    $sheet->setCellValue('H' . $row, $table_10_2_record->url_manzili ?? 'N/A');
                    $sheet->setCellValue('I' . $row, $table_10_2_record->url_indexsatsiya ?? 'N/A');
                    $sheet->setCellValue('J' . $row, $table_10_2_record->mualliflar_soni ?? 'N/A');

                    if ($table_10_2_record->asos_file) {
                        $sheet->setCellValue('K' . $row, 'Yuklash');
                        $sheet->getCell('K' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_10_2_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_10_2: $writtenRows");
    }
}