...
class Table_1_7_1_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_1_7_1_record = $pointEntry->table_1_7_1;

            if ($table_1_7_1_record) {
                try {
                    // Nomer berish
                    ...
                    $sheet->setCellValue('C' . $row, $table_1_7_1_record->xorijiy_granti_buyurtma_nomi ?? 'N/A');
                    $sheet->setCellValue('D' . $row, $table_1_7_1_record->xorijiy_granti_buyurtma_summasi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_1_7_1_record->jami_summa ?? 'N/A');

                    if ($table_1_7_1_record->asos_file) {
                        $sheet->setCellValue('F' . $row, 'Yuklash');
                        $sheet->getCell('F' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_1_7_1_record->asos_file));
                    } else {
