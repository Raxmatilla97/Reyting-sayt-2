...
class Table_14_2_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_14_2_record = $pointEntry->table_14_2; // Tablitsiya relation to'g'ri formatda

            if ($table_14_2_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_14_2_record->nomi_sana ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_14_2_record->davlat_otm_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_14_2_record->tezis_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_14_2_record->konf_seminar_nomi ?? 'N/A');

                    if ($table_14_2_record->asos_file) {
                        $sheet->setCellValue('H' . $row, 'Yuklash');
                        $sheet->getCell('H' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_14_2_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_14_2: $writtenRows");
    }
}