...
class Table_3_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_3_record = $pointEntry->table_3; // Tablitsiya relation to'g'ri formatda
            ...
            if ($table_3_record) {
                try {
                    ...
                    $sheet->setCellValue('D' . $row, $table_3_record->magistr_darajasi_bergan_otm ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_3_record->magistr_darajasi_seriya_raqam ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_3_record->mutaxasislik_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_3_record->ishga_qabul_raqam_sana ?? 'N/A');

                    if ($table_3_record->asos_file) {
                        $sheet->setCellValue('H' . $row, 'Yuklash');
                        $sheet->getCell('H' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_3_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_3: $writtenRows");
    }
}