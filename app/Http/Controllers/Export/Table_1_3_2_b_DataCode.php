...
class Table_1_3_2_b_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_1_3_2_b_record = $pointEntry->table_1_3_2_b; // Tablitsiya relation

            if ($table_1_3_2_b_record) {
                try {
                    // Raqam qo'yish
                    ...
                    $sheet->setCellValue('D' . $row, $table_1_3_2_b_record->dotsent_dip_serya ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_1_3_2_b_record->dotsent_dip_nomer ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_1_3_2_b_record->ishga_raq_sana ?? 'N/A');

                    if ($table_1_3_2_b_record->asos_file) {
                        $sheet->setCellValue('G' . $row, 'Yuklash');
                        $sheet->getCell('G' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_1_3_2_b_record->asos_file));
                    } else {
