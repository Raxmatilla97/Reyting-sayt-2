...
class Table_1_6_1_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_1_6_1_record = $pointEntry->table_1_6_1;

            if ($table_1_6_1_record) {
                try {
                    // Nomer berish
                    ...
                    $sheet->setCellValue('D' . $row, $table_1_6_1_record->xorijiy_jirnal_davlat_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_1_6_1_record->ilmiy_jurnal_nomi ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_1_6_1_record->ilmiy_maqola_nomi ?? 'N/A');
                    $sheet->setCellValue('G' . $row, $table_1_6_1_record->nashr_yili_betlari ?? 'N/A');

                    // URL manzilini giperhavola qilish
                    $url = $table_1_6_1_record->url_manzili ?? 'N/A';
                    if ($url !== 'N/A') {
                    ...
                    $sheet->setCellValue('I' . $row, $table_1_6_1_record->mualliflar_soni ?? 'N/A');

                    if ($table_1_6_1_record->asos_file) {
                        $sheet->setCellValue('J' . $row, 'Yuklash');
                        $sheet->getCell('J' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_1_6_1_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_1_6_1: $writtenRows");
    }
}
