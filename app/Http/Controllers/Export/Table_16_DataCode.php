...
class Table_16_DataCode
{
    public function exportTableData($sheet, $pointUserDeportaments)
    {
        ...
        foreach ($pointUserDeportaments as $pointEntry) {
            $table_16_record = $pointEntry->table_16; // Tablitsiya relation to'g'ri formatda
            ...
            if ($table_16_record) {
                try {
                    ...
                    $sheet->setCellValue('D' . $row, $table_16_record->fond_buyurtma_nomi ?? 'N/A');
                    $sheet->setCellValue('E' . $row, $table_16_record->fond_buyurtma_summa ?? 'N/A');
                    $sheet->setCellValue('F' . $row, $table_16_record->jami_summa ?? 'N/A');

                    if ($table_16_record->asos_file) {
                        $sheet->setCellValue('G' . $row, 'Yuklash');
                        $sheet->getCell('G' . $row)->getHyperlink()->setUrl(asset('storage/' . $table_16_record->asos_file));
                    } else {
        ...
        // Log::info("Total rows written to table_16: $writtenRows");
    }
}